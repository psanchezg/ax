...
**Fork-local document.** It is not part of the upstream-bound commit series (like
`sonar-project.properties`, `.specify/` and `.claude/`). It lives in `docs/` so the manual
validation record sits next to the feature documentation.

## Validation status

| Level | Status |
|---|---|
| 0 — suite and the real runner, locally | ✅ verified (macOS/arm64) |
| 0.5 — real `dsh` against the generated `settings.yaml` | ✅ verified (this is how the `agent-default-model` bug surfaced) |
| 1 — control plane without a cluster | ✅ verified (macOS/arm64) |
| 2 — real sandbox (Kubernetes + Substrate) | ✅ verified (macOS/arm64, kind, gVisor); the Linux/amd64 run is pending |
...
3. **The agent *can* write outside `/workspace`** inside the sandbox. Substrate isolates the
   ...
   actors), but it does not restrict the actor's rootfs. SC-007 has to be read as "it cannot
   escape the sandbox", not as "it only writes to `/workspace`" (§3.4.4).
...
### Platform notes (validated on macOS/arm64; Linux/amd64)

The commands below were run on macOS on Apple Silicon. On Linux they are the same, except
for these:
...
#    success criterion (SC-003), not a problem.
...
| 4 | sandbox isolation (SC-007) | §3.4.4 |
| 5 | the no-`harness` path is untouched (SC-005, SC-009) | §3.4.5 |
| 6 | key rotation with no changes (SC-004) | §3.4.6 |
...
#### 3.4.4 · Isolation (SC-007): the actor does not escape the sandbox
...
proves the opposite. SC-007 asks that the actor cannot reach the host or other actors, and
...
#### 3.4.5 · A task with no `harness` activates nothing new (SC-005, SC-009)
...
change. That is what SC-004 asks for.
...
|---|---|---|---|
| SC-001 DSH goal end-to-end | §3.3 | ✅ pass | DSH boots inside the actor (`dsh --version` → `0.1.5-rc.3`), reads the generated `settings.yaml` and the goal is answered against the `Model` endpoint. Before the packaging fix the same command died with `Duplicate type name 'DSH_STARTUPINFOW'` |
| SC-002 loud failures, no fabrication | §1.1 (`TestClient_NoFallbackWithoutOptIn`, `TestGoogle_ErrorsAreTyped`, `TestAnthropic_TypedErrors`) | ✅ pass | a failing provider returns a typed error; no response is invented outside the explicit `DisableRemote` opt-in |
| SC-003 invalid provider rejected at apply | §2 (1) | ✅ pass | `unknown provider "gemini-flash" (valid providers: anthropic, google, openai)` |
| SC-004 rotation with no changes | §3.4.6 | ✅ pass | rotating the secret and recreating the actor puts the new value in the container, with no code or manifest change |
| SC-005 backward compatibility | §1.1 + §3.4.5 | ✅ pass | suite green, and a task with no `harness` block gets no settings and no model variables |
| SC-006 version interoperability | §1.1 (`TestWire_RoundTrip_PreservesUnknownFields`) | ✅ pass | the round trip preserves unknown fields |
| SC-007 isolation | §3.4.4 | ✅ pass | the actor runs its own gVisor kernel (`4.19.0-gvisor` against the node's `7.0.12-linuxkit`); a writable rootfs is the expected result of this check, not a failure |
| SC-008 first boot without npm | §3.3 (the `headless` profile ships inside the package) | ✅ pass | the image pre-bakes the CLI and the profile, so the sandbox needs no npm registry access |
| SC-009 Antigravity untouched | §3.4.5 | ✅ pass | with no `harness` declared, the resolved harness is Antigravity: the path from before |
...
Before exporting upstream, remember that the series has to pass the cherry-pick test again
(`git format-patch upstream/main..<branch>` + `git am` on a clean `upstream/main`), which is
what the constitution verifies.
