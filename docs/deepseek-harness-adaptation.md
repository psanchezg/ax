...
`Task` / `Gateway` / `Workspace` / `Model` RPC groups.
...
- `Gateway` egress allowlists (`ax.proto:171-182`) are the natural place to
  control which model endpoints a task may reach.
...
The default gateway allows `*:443` (`reconciler.go:194-198`), so nothing breaks
out of the box. A hardened deployment must allowlist the chosen endpoint per
port, because a blocked provider looks exactly like a hung agent:
    ...
    allowlist:
      hosts:
        ...
        - host: api.together.xyz
          port: 443
        # Self-hosted inference inside the cluster:
        - host: vllm.ax-system.svc.cluster.local
          port: 8000
```
